...
const f3 = () => {
    ...
    a3_res = a3.filter(item => {
        if (item > a3_from && item < a3_to) return true;
    })
...
const f4 = () => {
    let res = str4.split('').find(item => {
        if (item > a4_from && item < a4_to) return true;
    });
...
const f5 = () => {
    let res = a5.findIndex(item => {
        if (item > a5_from && item < a5_to) return true;
    });
...
const f6 = () => {
    let res_6 = a6.reverse().findIndex(item => {
        if (item > a6_from && item < a6_to) return true;

    });

    a6.reverse();

    if (res_6 === -1) res_6 = false;
    else res_6 = (a6.length - 1) - res_6;

    document.querySelector('.out-6').innerHTML = res_6;
}
...
const f8 = () => {
    let res = str8.lastIndexOf(a8_1.toUpperCase());
