...
function t3() {
    let elem = document.querySelector('.ch-3');
...
function t4() {
    let elemAll = document.querySelectorAll('.r-4');
    for (let item of elemAll) {
...
function t8(num, method) {
    ...
    else if (method == 'floor') return Math.floor(num);
}
...
function t11() {
    let summ = Array.from(arguments).reduce((accum, item) => accum + item);
    return summ;
}
...
function t12(...arr) {
    let summ = arr.reduce((accum, item) => accum + item);
    return summ;
}
...
function t17() {
    let ind = document.querySelector("select.s-171").options.selectedIndex;
    let elem = document.querySelector("select.s-171").options[ind];
    return elem.value;
}
...
function t21(s) {
    while (true) {
        ...
        else if (s == 'odd' && rand % 2 !== 0) return rand;
    }
}
...
function t26() {
    let val = document.querySelector('.i-26').value;
    val = val.trim();
    if (val === '') return;
    document.querySelector('.out-26').textContent = val.toLowerCase();
}
