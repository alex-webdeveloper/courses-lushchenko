...
function t1(elem) {
    ...
    function r1(element) {
        out += element.tagName + ' ';
        let htmlCollection = element.children;
        if (htmlCollection.length > 0) Array.from(htmlCollection).forEach(item => r1(item));
    }
...
function t2(elem) {
    ...
    function r2(element) {
        let htmlCollection = element.children;
        if (htmlCollection.length > 0) Array.from(htmlCollection).forEach(item => r2(item));
        else if (!isNaN(+element.textContent)) s += +element.textContent;
    }
...
function t3(elem) {
    ...
    function r3(element) {
        let htmlCollection = element.children;
        if (htmlCollection.length === 0) out += element.textContent;
        else Array.from(htmlCollection).forEach(item => r3(item));
    }
...
function t4(elem) {
    ...
    function r4(element) {
        let htmlCollection = element.children;
        if (htmlCollection.length === 0) element.style.background = 'orange';
        else Array.from(htmlCollection).forEach(item => r4(item));
    }
}
...
function t5(elem) {
    ...
    function r5(element) {
        let htmlCollection = element.children;
        if (htmlCollection.length === 0) element.outerHTML = `<mark>${element.textContent}</mark>`;
        else Array.from(htmlCollection).forEach(item => r5(item));
    }
}
