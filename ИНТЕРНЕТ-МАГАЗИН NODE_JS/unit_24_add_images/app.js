...
let adminModule = require('./adminMod');
let hashModule = require('./hashMod');
/**
...
app.use(express.urlencoded({ extended: true })); // получение данных из post запроса без fetch
app.use(cookieParser()); // получение кук
...
app.get('/goods/*', function (req, res) {
  ...
  con.query('SELECT * FROM goods WHERE slug="' + req.params['0'] + '"', function (error, result, fields) {
    ...
    // if (result[0]['id'] != underfined)
    con.query('SELECT * FROM images WHERE goods_id=' + result[0]['id'], function (error, resultImages, fields) {
      if (error) throw error;
      console.log(resultImages);
      // if (resultImages.length != 0)// сделать проверку!
      resultImages = JSON.parse(JSON.stringify(resultImages));
      res.render('goods', { goods: result, goods_images: resultImages });
    });
  });
});
...
app.get('/admin-order', function (req, res) {
  ...
  LEFT JOIN	
    user_info
  ON shop_order.user_id = user_info.id ORDER BY date DESC`, function (error, result, fields) {
    if (error) throw error;
    // console.log(result);
  ...
  con.query(
    ...
    function (error, result) {
      ...
      if (result.length == 0) {
        ...
        //    * write hash to db
        //    */
        sql = "UPDATE users SET hash='" + hash + "' WHERE id=" + result[0]['id'];
