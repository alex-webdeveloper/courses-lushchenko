...
app.get('/goods', function (req, res) {
  ...
  con.query('SELECT * FROM goods WHERE id=' + req.query.id, function (error, result, fields) {
    ...
    else if (result.length > 0) res.render('goods', { goods: JSON.parse(JSON.stringify(result)) });
    else res.end('404');
  });
});
...
async function sendMail(data, result) {
  ...
  let mailOption = {
    ...
    to: "example@gmail.com," + data.email,
...
// INSERT INTO`category`(`id`, `category`, `description`, `image`) VALUES('Нотебуки', 'Ноутбук (англ. notebook — блокнот) — переносной компьютер, в корпусе которого объединены типичные компоненты ПК, включая дисплей, клавиатуру и устройство указания (обычно сенсорная панель или тачпад), а также аккумуляторные батареи.', '114290.120x150.jpg'),
//   ('Телефоны', 'Жизнь современного человека невозможно представить без мобильного устройства – с тех пор, как мобильный интернет получил широкое распространение, пользователи стали проводить перед экраном смартфона', '75093.120x150.png')
