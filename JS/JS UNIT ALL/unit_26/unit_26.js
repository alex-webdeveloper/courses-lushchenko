...
function fetchGet(div, arr) {
    fetch(`${url}?auth=${auth}${string(arr)}`)
        .then(response => {
            console.log(response);
            if (response.ok === true && response.status === 200) return response.text();
        })
        .then(data => {
            let arr = data.split('.');
            console.log(arr);
            if (arr.includes('png')) document.querySelector(div).innerHTML = `<img src="${data}" alt="">`;
            // if (div == '.out-7') document.querySelector(div).innerHTML = `<img src="${data}" alt="">`;
...
function fetchPost(div, arr) {
    fetch(url, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/x-www-form-urlencoded',
        },
        body: `auth=${auth}${string(arr)}`,
    })
        .then(response => {
            console.log(response);
            if (response.ok == true && response.status == 200) return response.text();
        ...
        .then(data => {
            ...
            if (div == '.out-16') document.querySelector(div).innerHTML = `<img src="${data}" alt="">`;
            console.log(data);
        });
}
// Task 1 ============================================
