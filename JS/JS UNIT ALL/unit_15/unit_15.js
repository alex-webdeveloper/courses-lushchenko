...
const f3 = () => {
    let inp = document.querySelector('.i-3');
    s3.delete(inp.value);
    console.log(s3);
}
...
const f4 = () => {
    ...
    document.querySelector('.out-4').textContent = s4.has(inp.value);
}
...
const f7 = () => {
    ...
    let out = 0;
    let set = new Set(v);
    if (v.length > 6 && v.length === set.size) {
        out = 1;
    }
...
const f8 = () => {
    ar8 = [];
    for (let item of s8) {
        if (typeof item == 'number' && item > 5) ar8.push(item);
    }
    console.log(ar8);
}
