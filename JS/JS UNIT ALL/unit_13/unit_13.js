...
function f15() {
    ...
    //     }
    // }
    // out = Object.keys(a15).map(item => a15[item].join(' ')).join(' ');
    out = Object.values(a15).flat().join(' ');
...
function f16() {
    ...
    // out = Object.keys(a16).map(item => a16[item]['name']).join(' ');
...
function f18() {
    let out = '';
    let k = document.querySelector('.i-18').value;
    for (let key in a18) {
        // if (a18[k] == a18[key]) out += a18[k].join(' ');
        if (k == key) out += a18[key].join(' ');
    }
...
function f19() {
    let out = '';
    let v = document.querySelector('.i-19').value;
    for (let key in a19) {
        for (let i = 0; i < a19[key].length; i++) {
            if (v.toLowerCase() == a19[key][i].toLowerCase()) out = key;
        }
    }
...
function f20() {
    ...
    for (let key in a20) {
        for (let i = 0; i < a20[key].length; i++) {
            ...
            if (a20[key][i].includes(2)) out += a20[key][i][0] + ' ';
        }
    }
